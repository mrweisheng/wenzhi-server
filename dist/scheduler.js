...
exports.scheduleCommissionFix = exports.dailyCommissionFix = exports.scheduleMissedOrderCheck = exports.dailyMissedOrderCheck = exports.autoFixSettlementStatus = exports.scheduleSettlementStatusSync = exports.initialCustomerOrderSync = exports.scheduleCustomerOrderSync = void 0;
...
// 结算状态自动修正定时任务
const scheduleSettlementStatusSync = () => {
    // 每4小时执行一次
    const FOUR_HOURS_MS = 4 * 60 * 60 * 1000;
    setInterval(async () => {
        try {
            console.log('执行结算状态自动修正任务...');
            await (0, exports.autoFixSettlementStatus)();
        }
        catch (error) {
...
exports.scheduleSettlementStatusSync = scheduleSettlementStatusSync;
// 自动修正结算状态
const autoFixSettlementStatus = async () => {
    ...
    while (hasMore) {
        ...
        const [orders] = await pool.query(`SELECT co.id, co.order_id, co.order_amount, co.settlement_status, co.is_fixed, o.amount
       ...
       LIMIT ? OFFSET ?`, [BATCH_SIZE, offset]);
        ...
        if (eligibleUpdates.length > 0) {
            await pool.query('UPDATE customer_orders SET settlement_status = ? WHERE id IN (?)', ['Eligible', eligibleUpdates]);
        }
        // 批量更新 Pending 状态
        if (pendingUpdates.length > 0) {
            await pool.query('UPDATE customer_orders SET settlement_status = ? WHERE id IN (?)', ['Pending', pendingUpdates]);
        }
...
exports.autoFixSettlementStatus = autoFixSettlementStatus;
/**
 * 每日遗漏订单检查任务
 * 处理因时序问题导致的遗漏订单（客服先录入，平台订单后同步的情况）
 */
const dailyMissedOrderCheck = async () => {
    const pool = require('./config/db').default || require('./config/db');
    ...
    try {
        // 动态导入函数，避免循环依赖
        const { checkEligibleForSettlement } = await Promise.resolve().then(() => __importStar(require('./controllers/customerOrder')));
...
exports.dailyMissedOrderCheck = dailyMissedOrderCheck;
// 启动遗漏订单检查定时任务
const scheduleMissedOrderCheck = () => {
    // 每天凌晨2点执行一次
    ...
    setTimeout(() => {
        (0, exports.dailyMissedOrderCheck)(); // 立即执行一次
        setInterval(async () => {
            try {
                console.log('执行遗漏订单检查任务...');
                await (0, exports.dailyMissedOrderCheck)();
            }
            catch (error) {
...
exports.scheduleMissedOrderCheck = scheduleMissedOrderCheck;
/**
 * 每日佣金修复任务
 * 检查并修复符合条件但佣金为0的订单
 */
const dailyCommissionFix = async () => {
    const pool = require('./config/db').default || require('./config/db');
    ...
    try {
        // 动态导入函数，避免循环依赖
        const { recalculateCustomerCommissionForOrder } = await Promise.resolve().then(() => __importStar(require('./controllers/customerOrder')));
...
exports.dailyCommissionFix = dailyCommissionFix;
// 启动佣金修复定时任务
const scheduleCommissionFix = () => {
    // 每天晚上12点执行一次
    const DAILY_MS = 24 * 60 * 60 * 1000;
    // 计算到今晚12点的延迟时间
    const now = new Date();
    ...
    setTimeout(() => {
        (0, exports.dailyCommissionFix)(); // 立即执行一次
        setInterval(async () => {
            try {
                console.log('执行佣金修复任务...');
                await (0, exports.dailyCommissionFix)();
            }
            catch (error) {
...
exports.scheduleCommissionFix = scheduleCommissionFix;
